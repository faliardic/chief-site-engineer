...
`SQLite`: Ayri bir database server gerektirmeden uygulama sureci icinde calisan, veriyi genellikle tek dosyada saklayan iliskisel veritabani.

`Schema`: Veritabanindaki tablolarin, alanlarin, iliskilerin ve constraint'lerin yapisini tanimlayan sozlesme.

`Schema Version`: Veritabani yapisinin hangi migration seviyesinde oldugunu gosteren surum numarasi.

`Migration`: Veritabani schema'sini sirali ve kontrollu bicimde bir surumden sonraki surume tasiyan degisiklik.

`Migration Runner`: Uygulanmis migration surumlerini okuyup pending migration'lari sirasiyla calistiran kod.

`Transaction`: Birden fazla veritabani islemini tek bir is birimi olarak ele alan sinir.

`Atomicity`: Bir transaction'in ya tamamen basarili olmasi ya da hic uygulanmamis sayilmasi ozelligi.

`Rollback`: Basarisiz transaction icinde yapilan degisiklikleri geri alma islemi.

`Idempotency`: Bir islemin tekrar calistirilmasinin ilk basarili sonucunu bozmamasi veya ikinci bir kopya olusturmamasi ozelligi.

`UUID`: Kayitlar icin cok dusuk cakisma riskiyle benzersiz kimlik saglayan 128-bit identifier standardi.

`Canonical UUID String`: UUID degerinin kucuk harfli, tireli ve ek parantez ya da bosluk icermeyen standart metin bicimi.

`UTC`: Farkli saat dilimlerindeki zamanlari ortak bir referansta gosteren evrensel zaman standardi.

`Optimistic Concurrency`: Kaydi uzun sure kilitlemek yerine okunan revision ile guncel revision'i karsilastirarak cakisan update'i tespit etme yaklasimi.

`Revision`: Bir kaydin hangi guncelleme surumunde oldugunu gosteren ve optimistic concurrency kontrolunde kullanilan sayac.

`Managed File Store`: Dosyalarin uygulamanin belirledigi bir root altinda adlandirma, butunluk ve lifecycle kurallariyla saklandigi alan.

`Relative Path`: Bir dosyanin mutlak disk konumu yerine yonetilen root'a gore yazilan tasinabilir yolu.

`SHA-256`: Dosya iceriginin degisip degismedigini kontrol etmek icin 256-bit ozet ureten cryptographic hash algoritmasi.

`Lifecycle Metadata`: Bir kaydin active, archived, superseded veya missing gibi yasam dongusu durumunu anlatan metadata.

`Reconciliation`: Veritabani metadata'si ile file store gibi iki iliskili kaynagi karsilastirip uyusmazliklari bulma sureci.

`ADR`: Architecture Decision Record; onemli bir teknik kararin baglamini, secimini, sonuclarini ve kapsam disini kaydeden dokuman.

`PRAGMA`: SQLite baglantisi veya database davranisini ayarlamak ve sorgulamak icin kullanilan ozel komut ailesi.

`Failure Injection`: Rollback ve hata yonetimi gibi olumsuz yollari test etmek icin kontrollu bicimde hata uretme teknigi.
